...
// Require 'Log' model in our routes module
...
logRoutes.route('/add').post(function (req, res) {
  ...
  log.save()
    .then(log => {
      res.status(200).json({'log': 'Log is added successfully'});
    })
    .catch(err => {
...
// Defined edit route
logRoutes.route('/edit/:id').get(function (req, res) {
...
logRoutes.route('/search/:asset_serial').get(function (req, res) {
...
logRoutes.route('/showLogID/:log_type').get(function (req, res) {
...
logRoutes.route('/search/:log_type/:from_date/:to_date').get(function (req, res) {
  let log_type = req.params.log_type;
