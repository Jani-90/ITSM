...
export class AssetAuditTrailService {

  uri = 'http://localhost:4000/assetAuditTrail';

  constructor(private http: HttpClient) { }

  addAuditTrail(asset_code, asset_serial, asset_barcode, action, currentEmployeeNo, currentEmployeeName) {
    const obj = {
        ...
        create_by_name: currentEmployeeName,
  ...
  showAuditTrail(asset_serial) {
    //asset_code = encodeURIComponent(asset_code);
    return this
            .http
            .get(`${this.uri}/showAuditTrail/${asset_serial}`);
    }
